...
import { removeFromFavorites } from "../../../lib/supabaseQueries";
import { useFavorites } from "../../../api/supabase/hooks";
import MovieCard from "../movies/MovieCard";
import { Heart, Search } from "lucide-react";
import { filterFilmItems } from "./filterFilmItems";

function ProfileFavoritesTab({ searchQuery }) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  const { data: items = [], isLoading, error } = useFavorites(user?.id, {
    enabled: !!user,
  });
  ...
  const handleRemove = async (kinopoiskId) => {
    ...
    try {
      ...
      queryClient.setQueryData(["favorites", user.id], (old) =>
        (old || []).filter((fav) => fav.kinopoisk_id !== kinopoiskId)
      );
    } catch (err) {
      console.error(err);
    }
